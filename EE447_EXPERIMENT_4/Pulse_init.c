...
#define LOW 	0x32				// 50 cycles, load 50
#define HIGH 	0x28				// load 40
...
void pulse_init(void){
  ...
  GPIOF->DEN       |= 0x08;  // Enable PF3 pin as a digital pin
	...
	TIMER0 ->TAMATCHR = HIGH;				//load match register
	TIMER0 ->TAMR |= 0x20; 
	TIMER0 ->IMR |= 0x10;
	GPIOF ->DATA |= 0x08;				//initialize PF3
...
void TIMER0A_Handler (void){
	...
	TIMER0 ->ICR |= 0x10;			//clear match interrupt
	return;
}
