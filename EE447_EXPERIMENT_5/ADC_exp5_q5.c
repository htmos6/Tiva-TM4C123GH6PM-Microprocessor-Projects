...
void delayInSeconds(float time);
...
void initializationOfADC0(void)
{
  // Configurations of the PE3 as analog input
	SYSCTL->RCGCGPIO |= 0x30;     // Enable clock for Port E & F
	__ASM("NOP");
	__ASM("NOP");
	__ASM("NOP");
	GPIOE->DIR &= ~0x08u;
  ...
  GPIOE->DEN &= ~0x08u;    			// Disable digital on PE3
	...
	GPIOF->DIR |= 0x02u;						// PF1 as output
	GPIOF->AMSEL &= ~0x02u;   			// Disable analog on PF1
	GPIOF->AFSEL &= ~0x02u;   			// Disable alternate function on PF1
  GPIOF->DEN |= 0x02u;    				// Enable digital on PF1
...
int main(void)
{
  ...
  // Call initialize function for ADC initialization
	...
	{
		delayInSeconds(0.1); // 1 second delay
