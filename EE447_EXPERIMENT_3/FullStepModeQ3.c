...
void GPIOB_Handler (void){  // When interrupt comes from push buttons, handle with it. 
	...
	if(data != 0xF)
	{
		...
		{//no bouncing
			switch(data) 
			{
				case 14: //1110 PINB4 //clockwise 0
					pressed_temp = 0;		// Store pressed button inside temp. 		
					break;
				case 13: //1101 PINB5 //counterclockwise 1
					//counterclockwise 1  PINB5
	...
	else{ // Buttons [7:4] = 1111 xxxx meaning that buttons are released. 
		data2 = GPIOB -> DATA;
		data2 &= 0xF0;			//clear the step motor output bits
		data2 = (data2 >> 4);
		...
		if(data == data2)
		{
			if(pressed_temp < 2)
			{
				dir = pressed_temp; // When button is released, change direction of the motor. 			
			}
			else if(pressed_temp == 2)  // If button 2 is pressed, run faster
			{
				init_SysTick(8777215);
			}
			else
			{
				init_SysTick(1000000); // Run slower 
			}
		}
	}
